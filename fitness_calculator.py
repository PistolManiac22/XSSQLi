...
⭐ CRITICAL BUG FIX: Payload Normalization
- Payloads come URL-encoded from GA mutations
- Must DECODE them before appearance detection
- %3Cmarquee... → <marquee... for proper classification
...
⭐ FIX bWAPP HIGH false positives:
- METHOD 2 keyword fallback hanya trigger untuk RAW/PARTIAL_URL
- TIDAK trigger untuk SINGLE_URL/MULTI_URL (aman dari browser)
"""
...
class GAXSS_FitnessCalculator:
    ...
    def _detect_how_payload_appears_in_response(
        self, payload: str, response: str
    ) -> str:
        """Detect HOW payload appears in SERVER RESPONSE.

        ⭐ CRITICAL FIX: Decode URL-encoded payloads first!
        ...
        - MULTI_URL: %253Csvg%2520onerror%253D  (NON-EXECUTABLE)
        ...
        # ⭐⭐⭐ CRITICAL FIX: STEP 0 - Payload Normalization ⭐⭐⭐
        # Payloads from GA mutations are often URL-encoded
        # We must decode them to check if they're truly "raw" in response
        original_payload = payload
        decoded_payload = payload
        
        try:
            # Try to URL-decode the payload
            decoded = urllib.parse.unquote(payload)
            
            # If decoding changed it, payload was URL-encoded
            if decoded != payload and "%" in payload:
                logger.debug(
                    f"[DECODE] Payload was URL-encoded: {payload[:40]}... → {decoded[:40]}..."
                )
                decoded_payload = decoded
        except Exception as e:
            logger.debug(f"[DECODE] Could not decode payload: {e}")
            decoded_payload = payload
        
        # ⭐⭐⭐ END FIX ⭐⭐⭐

        # STEP 1: Check if DECODED payload appears RAW (as-is) in response
        # This catches truly executable payloads
        if decoded_payload in response:
            # Verify it's not URL-encoded in the response context
            idx = response.find(decoded_payload)
            if idx > 0:
                # Check surrounding context for encoding markers
                context_start = max(0, idx - 10)
                context = response[context_start:idx]
                if "%" not in context:
                    logger.debug("[DETECT] Decoded payload appears RAW in response")
                    return "raw"

        # STEP 2: Check if server HTML-encoded it
        if "<" in decoded_payload or ">" in decoded_payload:
            if "&lt;" in response or "&gt;" in response:
                ...
                sample = (
                    html_encoded[:50]
                    if len(html_encoded) > 50
                    else html_encoded
                )
                if sample in response:
                    logger.debug(
                        "[DETECT] Payload appears HTML-encoded in response"
                    )
                    return "html_encoded"

        # STEP 3: Check URL encoding status of ORIGINAL payload in response
        # If original encoded form is in response, classify by encoding layers
        if original_payload in response and original_payload != decoded_payload:
            # Original (encoded) payload found in response
            logger.debug(
                f"[DETECT] Original URL-encoded payload found in response"
            )
            
            # Count encoding layers
            ...
            if encoding_layers >= 2:
                logger.debug(
                    f"[DETECT] => 'multi_url' (multi-layer encoding, non-executable)"
                )
                return "multi_url"
            else:
                # Single layer URL encoding = non-executable
                logger.debug(
                    "[DETECT] => 'single_url' (single-layer URL encoding, non-executable)"
                )
                return "single_url"

        # STEP 4: Check URL encoding status in response
        # Even if our exact payload not found, check for encoded angle brackets
        if (
            "%3C" in response
            or "%3c" in response
            or "%3E" in response
            ...
            if has_raw_brackets:
                ...
                # Extract potential tag names from decoded payload
                tag_match = re.search(r"<(\w+)", decoded_payload)
                if tag_match:
                    ...
                    if (
                        encoded_tag in response
                        or encoded_tag_lower in response.lower()
                    ):
                        logger.debug(
                ...
                logger.debug(
                    "[DETECT] Only URL-encoded brackets found => SINGLE_URL encoding (non-executable)"
                )
                return "single_url"

        # STEP 5: Check if server kept it Base64-encoded
        try:
            base64_payload = base64.b64encode(decoded_payload.encode()).decode()
        ...
        # STEP 6: Not found in any recognizable form
        logger.debug("[DETECT] Payload not found in response")
        return "not_found"

    # ==================== COMPONENT 1: EXECUTION SUCCESS ====================

    def _find_payload_window(
        self, payload: str, text: str, window_size: int = 300
    ) -> str:
    ...
    def _detect_code_execution(self, payload: str, response: str) -> bool:
        ...
        appearance = self._detect_how_payload_appears_in_response(
            payload, response
        )
        ...
        if appearance == "single_url":
            logger.debug(
                "[DEBUG-EX] Server kept SINGLE URL-encoding => Non-executable => Ex=0"
            )
            return False

        if appearance == "multi_url":
            logger.debug(
                "[DEBUG-EX] Server kept MULTI-layer URL-encoding => Non-executable => Ex=0"
            )
            return False

        if appearance == "base64":
            logger.debug(
                "[DEBUG-EX] Server kept Base64 encoding => Non-executable => Ex=0"
            )
            return False
        ...
        logger.debug(
            f"[DEBUG-EX] Payload potentially executable (appearance={appearance})"
        )
        ...
        baseline_window = self._find_payload_window(
            payload, self.baseline_tail_after_marker or ""
        )
        ...
        # ⭐ FIX: HANYA trigger jika payload RAW atau PARTIAL_URL (benar-benar bisa execute)
        # JANGAN trigger untuk SINGLE_URL/MULTI_URL encoding (aman dari browser)
        if appearance in ["raw", "partial_url"]:
            dangerous_keywords = [
                "<script",
                "<img",
                "<iframe",
            ...
            if len(new_keywords) >= 2:
                ...
                matched = [
                    kw for kw in new_keywords if kw.strip("<>=") in payload_lower
                ]
    ...
    def _count_unclosed_symbols(self, text: str) -> Tuple[int, int]:
        ...
        total_symbols = (
            open_angle + close_angle + text.count('"') + text.count("'")
        )
    ...
    def calculate_closed(self, payload: str, response: str) -> float:
        ...
        if payload_idx == -1:
            ...
            context_end = min(
                len(response), payload_idx + len(payload) + 100
            )
    ...
    def calculate_dis(self, payload: str, response: str) -> float:
        ...
        distance = self._levenshtein_distance(
            payload_sample, response_sample
        )
    ...
    def calculate_pu(self, payload: str, response: str) -> float:
        ...
        if self.baseline_length > 0:
            length_change = abs(len(response) - self.baseline_length) / float(
    ...
    def calculate_fitness(
        ...
        try:
            ...
            fitness = (
                w1 * (ex / 2.0) + w2 * closed + w3 * dis - w4 * pu
            )
    ...
    def payload_is_xss_vector(payload: str) -> bool:
        ...
        return bool(
            (has_tag or has_script)
            and (has_event or has_script or has_javascript)
        )