...
Examples (bWAPP):
  python main_gaxss.py xss -u "http://localhost:8082/xss_get.php" --bwapp --auto-discover --security low

Examples (Mutillidae):
  python main_gaxss.py xss -u "http://localhost:9000/index.php?page=user-info.php" --mutillidae --auto-discover
            """
        )
        ...
        # Mutillidae-specific options
        mutillidae_group = xss_parser.add_argument_group('Mutillidae Options')
        mutillidae_group.add_argument('--mutillidae-security', default='0',
                                     choices=['0', '1', '2', '3', '4', '5'],
                                     help='Mutillidae security level 0-5 (default: 0)')
        ...
        if args.mode == 'xss':
            if args.generic:
                ...
                webapp_config = DVWAConfig(base_url=base_url, username=args.username, password=args.password, security_level=args.security)
                ...
                webapp_config = BWAPPConfig(base_url=base_url, username=args.bwapp_user, password=args.bwapp_pass, vulnerability=args.vulnerability, security_level=args.security)
                ...
                base_url_match = re.match(r'(https?://[^/]+)', args.url)
                base_url = base_url_match.group(1) if base_url_match else "http://localhost:9000"
                webapp_config = MutillidaeConfig(base_url=base_url, security_level=args.mutillidae_security)
                self.logger.info(f"Using Mutillidae configuration at {base_url}")
                ...
                self.logger.error("Must specify --generic, --dvwa, --bwapp, --mutillidae, or --custom-url")
                parser.print_help()
                return
