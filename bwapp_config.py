...
class BWAPPConfig(WebAppConfig):
    ...
    def __init__(
        ...
        # param -> "GET"/"POST", diisi oleh CLI dari ParameterDiscoverer
        self.param_methods: Dict[str, str] = getattr(self, "param_methods", {})
    ...
    def authenticate(self) -> Optional[requests.Session]:
        ...
        try:
            ...
            _ = self.session.get(login_url)
            self.logger.info(
                f"[DEBUG] Initial cookies: {self.session.cookies.get_dict()}"
            )
            ...
            self.logger.info(
                f"[DEBUG] Cookies after login: {self.session.cookies.get_dict()}"
            )

            if "Logout" in r.text or "portal.php" in r.url or "Welcome" in r.text:
                self.logger.info("[OK] bWAPP logged in successfully")

                # Pastikan cookie security_level ada dan sesuai
                if "security_level" not in self.session.cookies:
                    self.logger.info(
                        f"[DEBUG] Setting security_level cookie to: {self.security_level_code}"
                    )
                ...
                self.logger.info(
                    f"[DEBUG] Final cookies (dict): {self.session.cookies.get_dict()}"
                )
                self.logger.info("=" * 60)
                return self.session
                ...
                self.logger.error(
                    "[ERROR] Login failed - no 'Logout' or 'Welcome' in response"
                )
    ...
    def send_payload(self, url: str, param: str, payload: str) -> str:
        ...
        try:
            ...
            self.logger.debug(
                f"[DEBUG] Payload (first 50 chars): {payload[:50]}..."
            )
            self.logger.debug(
                f"[DEBUG] Current security_level cookie: {current_level}"
            )
            ...
            if current_level != self.security_level_code:
                ...
                self.logger.info(
                    f"[DEBUG] Re-set security_level to: {self.security_level_code}"
                )
            ...
            # Pilih method berdasarkan param_methods (hasil discovery), default GET
            method = "GET"
            if hasattr(self, "param_methods"):
            ...
            # Beberapa hint sederhana untuk XSS (boleh dibiarkan seperti semula)
            if "&lt;" in response.text and "&gt;" in response.text:
                self.logger.debug(
                    "[DEBUG] Response contains HTML entities (likely encoded/safe)"
                )
            elif "<" in response.text and ">" in response.text and param in url:
                self.logger.debug(
                    "[DEBUG] Response contains raw HTML tags (likely unencoded)"
                )
    ...
    def get_global_params(self) -> Dict[str, str]:
        ...
        params: Dict[str, str] = {}
        if self.vulnerability.startswith("xss"):
            params.update(
                {"form": "submit", "firstname": "test", "lastname": "test"}
            )

        if self.vulnerability.startswith("sqli"):
            # Untuk semua SQLi bWAPP, biasanya perlu action=search
            params.update({"action": "search"})
        return params
